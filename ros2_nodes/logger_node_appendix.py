...
Key columns:
  rc_throttle     raw stick value (-1 to +1)
  local_vz        vertical velocity EKF2 (NED: +down)
  local_z         altitude EKF2 (NED: negative = above ground)
  of_vel_body_x/y body-frame velocity from EKF2 optical flow processing
  fused_quality   quality of fused_optical_flow arriving at PX4
"""
...
class UavLogger(Node):
    def __init__(self):
        ...
        self.state = {
            ...
            'rc_roll':          None,
            'rc_pitch':         None,
            'rc_throttle':      None,   # -1=down, 0=center, +1=up
            'rc_yaw':           None,
            'rc_valid':         None,

            # Controller attitude setpoint sent to PX4
            'cmd_roll_deg':     None,
            'cmd_pitch_deg':    None,
            'cmd_yaw_deg':      None,
            'cmd_thrust':       None,   # 0–1, positive = up

            # Fused optical flow published to PX4
            'fused_pixel_flow_x':  None,
            'fused_pixel_flow_y':  None,
            'fused_quality':       None,
            'fused_distance_m':    None,
            'fused_dist_avail':    None,

            # Raw sensor_optical_flow (last seen)
            'raw_device_id':    None,
            'raw_quality':      None,
            'raw_pixel_flow_x': None,
            ...
            'local_vx':         None,
            'local_vy':         None,
            'local_vz':         None,   # NED: positive = moving down
            'local_xy_valid':   None,
            'local_v_xy_valid': None,
            'local_z_valid':    None,
            'local_dead_reck':  None,
        ...
        self.row_count = 0
        self.create_timer(0.1,  self.log_row)        # 10 Hz
