...
NAV_NAMES = {
    0:'MANUAL', 1:'ALTCTL', 2:'POSCTL', 3:'MISSION', 4:'LOITER',
...
def _ok(t, lim=TOPIC_STALE_S):
    a = _age(t)
    return a is not None and a < lim
...
def _tick(ok):
    return Text('● OK  ', style='green') if ok else Text('● FAIL', style='red')
...
class DashboardNode(Node):
    def __init__(self):
        ...
        self.fused_q   = None
        self.fused_pf  = [None, None]
        self.fused_dist= None
        self.fused_dok = None

        self.raw_q     = {}   # device_id → quality

        self.of_body   = [None, None]
        self.of_ne     = [None, None]

        self.pos       = [None, None, None]
        self.vel       = [None, None, None]
        self.xy_valid  = None
        self.v_xy_valid= None
        self.z_valid   = None
        self.eph       = None
        self.evh       = None
        self.dead_reck = None
    ...
    def _cb_fused(self, m):
        ...
        self.fused_q    = m.quality
        self.fused_pf   = [m.pixel_flow[0], m.pixel_flow[1]]
        self.fused_dist = m.distance_m
        self.fused_dok  = m.distance_available

    def _cb_raw(self, m):
        self.t['raw'] = time.time()
        self.raw_q[m.device_id] = m.quality
    ...
    def _cb_local(self, m):
        ...
        self.xy_valid   = m.xy_valid
        self.v_xy_valid = m.v_xy_valid
        self.z_valid    = m.z_valid
        self.eph        = m.eph
        self.evh        = m.evh
        self.dead_reck  = m.dead_reckoning
        self.vxy_max    = m.vxy_max

    def _cb_status(self, m):
        self.t['status'] = time.time()
        self.nav_state   = m.nav_state
        self.arm_state   = m.arming_state

    def _cb_rc(self, m):
        self.t['rc']  = time.time()
        self.rc_thr   = m.throttle
        self.rc_roll  = m.roll
        self.rc_pitch = m.pitch
        self.rc_yaw   = m.yaw
        self.rc_valid = m.valid
    ...
    def _cb_dist(self, m):
        ...
        self.dist_m    = m.current_distance
        self.dist_q    = m.signal_quality

    def _cb_ctrl(self, m):
        self.t['ctrl'] = time.time()
        try:
            self.ctrl = json.loads(m.data)
        except Exception:
            pass
    ...
    def render(self):
        ...
        g1 = Table(box=box.SIMPLE, show_header=True,
        ...
        g2 = Table(box=box.SIMPLE, show_header=True,
        ...
        for i, did in enumerate(sorted(self.raw_q.keys())[:2]):
            ...
            g2.add_row(f'sensor {i+1} (0x{did:06x})',
                       _tick(raw_ok), f'Q=[{qc}]{q}[/]')
        ...
        g2.add_row('fused → PX4 (device 999999)',
                   _tick(fused_ok),
        ...
        g2.add_row('vehicle_optical_flow_vel', _tick(of_ok),
                   ...
                   f'ne=[{self.of_ne[0]:+.3f},{self.of_ne[1]:+.3f}]'
                   if self.of_body[0] is not None else '—')

        # Group 3: EKF2 state
        g3 = Table(box=box.SIMPLE, show_header=True,
        ...
        g3.add_row('EKF2 OF fused', _vtick(self.ekf_of),
                   f'innov=0x{self.ekf_innov:04x}'
                   if self.ekf_innov is not None else '—')

        dr_text = (Text('⚠ YES', style='red bold') if self.dead_reck
                   else Text('No', style='green'))
        g3.add_row('dead_reckoning', dr_text, '')

        vmax_ok = self.vxy_max is not None and self.vxy_max > 0.5
        g3.add_row('vxy_max',
                   Text(f'{self.vxy_max:.2f}m/s',
                        style='green' if vmax_ok else 'yellow')
                   if self.vxy_max else Text('—', style='grey50'),
                   '[yellow]low — fly higher[/]'
                   if not vmax_ok and self.vxy_max else '')
        ...
        # Group 4: Controller
        g4 = Table(box=box.SIMPLE, show_header=True,
                   header_style='bold green',
                   title='[bold green]Controller[/]')
        g4.add_column('Parameter', min_width=22)
        g4.add_column('Value',     min_width=32)

        nav_name  = NAV_NAMES.get(self.nav_state, f'nav={self.nav_state}')
        nav_color = 'green' if self.nav_state == 14 else 'yellow'
        arm_name  = ARM_NAMES.get(self.arm_state, f'arm={self.arm_state}')
        arm_color = 'green' if self.arm_state == 2 else 'grey50'
        g4.add_row('Flight mode',
                   f'[{nav_color}]{nav_name}[/]  [{arm_color}]{arm_name}[/]')

        rc_ok2 = _ok(self.t['rc']) and self.rc_valid
        if rc_ok2 and self.rc_thr is not None:
            tc = 'yellow' if abs(self.rc_thr) > 0.08 else 'green'
            g4.add_row('RC sticks (live)',
                       f'thr=[{tc}]{self.rc_thr:+.3f}[/]  '
                       f'roll={self.rc_roll:+.3f}  '
                       f'pitch={self.rc_pitch:+.3f}  '
                       f'yaw={self.rc_yaw:+.3f}')
        else:
            g4.add_row('RC sticks',
                       f'[red]NO SIGNAL[/]  age={_age_str(self.t["rc"])}')

        ctrl_ok = _ok(self.t['ctrl'])
        phase   = self.ctrl.get('phase', '—')
        in_ob   = self.ctrl.get('in_offboard', False)
        rc_fr   = self.ctrl.get('rc_fresh', False)
        g4.add_row('Controller status', _tick(ctrl_ok))
        g4.add_row('Phase',
                   f'[cyan]{phase}[/]  '
                   f'offboard=[{"green" if in_ob else "grey50"}]'
                   f'{"YES" if in_ob else "no"}[/]  '
                   f'RC fresh=[{"green" if rc_fr else "red"}]'
                   f'{"yes" if rc_fr else "NO"}[/]')

        if self.ctrl:
            vxc = self.ctrl.get('vx_cmd', 0)
            vyc = self.ctrl.get('vy_cmd', 0)
            vzc = self.ctrl.get('vz_cmd', 0)
            g4.add_row('Commands (m/s)',
                       f'vx={vxc:+.3f}  vy={vyc:+.3f}  '
                       f'vz=[{"yellow" if abs(vzc)>0.02 else "green"}]'
                       f'{vzc:+.3f}[/]')

            xt = self.ctrl.get('x_target')
            yt = self.ctrl.get('y_target')
            if xt is not None:
                px = self.ctrl.get('pos_x', 0)
                py = self.ctrl.get('pos_y', 0)
                g4.add_row('Position lock',
                           f'[green]LOCKED[/] target=({xt:.3f},{yt:.3f})  '
                           f'error=({xt-px:+.3f},{yt-py:+.3f})m')
            else:
                g4.add_row('Position lock', '[grey50]no lock[/]')

        # Layout
        top    = Columns([g1, g2], equal=False, expand=True)
        bottom = Columns([g3, g4], equal=False, expand=True)
        ts     = time.strftime('%H:%M:%S')
        return Panel(RGroup(top, bottom),
