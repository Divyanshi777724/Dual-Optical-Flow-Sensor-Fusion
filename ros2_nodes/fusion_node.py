#!/usr/bin/env python3
"""
fusion_node.py — Quality-weighted dual optical flow sensor fusion.
Publishes FusedOpticalFlow at 50 Hz to /fmu/in/fused_optical_flow for EKF2.
"""
...
# Physical sensor identity  verify with:
#   ros2 topic echo /fmu/out/sensor_optical_flow field device_id
DEVICE_ID_S1    = 0x847c03   # = 8682499  H-Flow unit 1 (body-frame aligned)
DEVICE_ID_S2    = 0x847d03   # = 8682755  H-Flow unit 2 (body-frame aligned)

# Physical mounting of sensor 2 relative to sensor 1 (degrees, yaw only).
# Both units are mounted aligned with the Pixhawk body frame.
SENSOR2_YAW_DEG = 0.0

# Virtual device ID written into FusedOpticalFlow.
# Must be unique and non-zero — identifies the fused source in PX4 logs.
FUSED_DEVICE_ID = 999999

# Quality gate: sensors below this threshold are excluded from fusion.
# Range 0–255. Typical good-light quality: 80 - 150.
MIN_QUALITY     = 50

# Quality penalty applied when only one sensor is available.
SINGLE_SENSOR_QUALITY_PENALTY = 20

# Optical flow sensor stale threshold. Extended to tolerate intermittent
# serial delays observed during flight.
FLOW_TIMEOUT_S  = 2.0

# Distance sensor stale threshold. Shorter since distance updates at ~50 Hz.
DIST_TIMEOUT_S  = 0.5

# Fusion output rate (Hz).
LOOP_HZ         = 50

# Pixel flow outlier rejection. Values outside this range per integration
# period are rejected before fusion.
MAX_FLOW_RAD    = 0.5   # rad per integration period (~16 ms)
...
class SensorBuf:
    """Holds the latest message from one optical flow sensor."""
...
class FusionNode(Node):
    ...
    def _cb_s1(self, msg: SensorOpticalFlow): self.s1.update(msg)
    def _cb_s2(self, msg: SensorOpticalFlow): self.s2.update(msg)
    ...
    def _fuse(self):
        ...
        if s1_ok and s2_ok:
            # ── BothValid: quality-weighted average ───────────────────────
            ...
            if da1 and da2:
                out.delta_angle = [float(w1*da1[i] + w2*da2[i]) for i in range(3)]
                out.delta_angle_available = True
            elif da1 or da2:
                out.delta_angle = da1 or da2
                out.delta_angle_available = True
            ...
            out.error_count = self.s1.msg.error_count + self.s2.msg.error_count
            self._cnt_both += 1

        elif s1_ok or s2_ok:
            # ── SingleOnly: copy valid sensor, apply quality penalty ───────
            ...
            # ── NoneValid: publish zeros — EKF2 rejects at quality gate ───
        ...
        # Distance is read from the dedicated rangefinder topic and embedded
        # into the FusedOpticalFlow message for EKF2 height scaling.
        if self._dist_fresh():
        ...
        if self._n % (LOOP_HZ * 5) == 0:
            pct_b = 100.0 * self._cnt_both   / max(1, self._cnt_total)
            pct_s = 100.0 * self._cnt_single / max(1, self._cnt_total)
            pct_n = 100.0 * self._cnt_none   / max(1, self._cnt_total)
            q1 = self.s1.msg.quality if self.s1.msg else 0
            q2 = self.s2.msg.quality if self.s2.msg else 0
            self.get_logger().info(
                f'Both:{pct_b:.0f}%  Single:{pct_s:.0f}%  None:{pct_n:.0f}%  '
